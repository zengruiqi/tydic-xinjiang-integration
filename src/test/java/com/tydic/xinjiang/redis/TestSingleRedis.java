package com.tydic.xinjiang.redis;
...
public class TestSingleRedis {
    ...
    public void testJson() {
        for (int i = 0; i < 10; i++) {
            String get = jedis.get("student");
            if (get != null && !get.equals("null")) {
                System.out.println("cache----");
                ;
            } else {
                Student student = queryData();
                String s = JSON.toJSONString(student);
                jedis.set("student", s);
                get = s;
            }
            Object object = JSON.parseObject(get, Student.class);
            System.out.println(object);
        }
    }

    @Test
    public void testString() { //-----添加数据---------- jedis.set("name","itlead");//向key-->name中放入了value-->xinxin
